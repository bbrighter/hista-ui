	...
	return {
		to: {
			...
			mealDetail: (mealId: number) =>
				navigateWithPiid(appRoutes.mealDetails, { mealId }),
			conditionEvents: () => navigateWithPiid(appRoutes.conditionEvents),
			conditionEventDetails: (eventId: number) =>
				navigateWithPiid(appRoutes.conditionEventDetails, { eventId }),
			...
			statusSingle: (id: number) =>
				navigateWithPiid(appRoutes.statusSingle, { id }),
		},
	};
};
