...
const getIngredientRow = (ingredient: string): HTMLElement => {
	...
	return ing.closest("li") as HTMLElement;
};
...
const getNutritionButton = (ingredient: string): HTMLElement => {
	return within(getIngredientRow(ingredient)).getByTestId(
		"editNutritionButton",
	);
};
...
describe("IngredientManagement", () => {
	...
	it("Archiving", async () => {
		...
		expect(archiveButton).toBeInTheDocument();
		expect(
			within(archiveButton).getByTestId("ArchiveIcon"),
		).toBeInTheDocument();
	...
	it("Manage nutrition", async () => {
		...
		const nutritionButton = await waitFor(() =>
			getNutritionButton("ingredient1"),
		);
