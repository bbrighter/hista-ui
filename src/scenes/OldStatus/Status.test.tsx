...
describe("Status.tsx", async () => {
	const getStatusCard = (date: string) =>
		screen.getByText(date).closest(".MuiPaper-root") as HTMLElement;
	const getDeleteButton = (date: string) =>
		within(getStatusCard(date)).getByTitle("Löschen");
	const getMorningFitnessSlider = (date: string) =>
		within(getStatusCard(date)).getByTestId("morning-fitness-slider");
	const getMorningSleepSlider = (date: string) =>
		within(getStatusCard(date)).getByTestId("morning-sleep-slider");
	const getEveningFitnessSlider = (date: string) =>
		within(getStatusCard(date)).getByTestId("evening-fitness-slider");
	const queryLockOverlay = (date: string) =>
		within(getStatusCard(date)).queryByTestId("lock-overlay");
	...
	it("Edit", async () => {
		...
		const eveningFitnessSlider = within(
			getEveningFitnessSlider(date),
		).getByRole("slider");
		expect(eveningFitnessSlider).toBeInTheDocument();
	...
	it("Unlock", async () => {
		...
		const date = "01.01.2024";
		const lockOverlay = await waitFor(
			() => queryLockOverlay(date) as HTMLElement,
		);
		expect(lockOverlay).toBeInTheDocument();
