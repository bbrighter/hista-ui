...
const getDeleteButton = (categoryName: string) => (findButtonWithinCategory(categoryName, 'Löschen'))
const getRenameButton = (categoryName: string) => (findButtonWithinCategory(categoryName, 'Kategorie umbenennen'))
const getExpandIcon = (categoryName: string) => (findButtonWithinCategory(categoryName, 'Ausklappen'))
...
describe('SymptomManagement', () => {
    ...
    it('deleting category', async () => {
        ...
        await waitFor(() => {
            deleteButton = getDeleteButton('cat (2)')
        })
        ...
        expect(screen.getByText('cat with no symptoms wirklich löschen?')).toBeInTheDocument()
