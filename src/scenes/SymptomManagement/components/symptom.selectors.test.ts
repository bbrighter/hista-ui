...
import {
	useIsCategoryNameAvailable,
	useIsSymptomNameAvailable,
} from "./useNamesAvailable";
...
describe("useIsSymptomNameAvailable", () => {
		...
		setSymptoms([
			{
				categoryId: 1,
				categoryName: "",
				symptoms: [{ categoryId: 1, id: 1, name: "name" }],
			},
			{
				categoryId: 2,
				categoryName: "",
				symptoms: [{ categoryId: 2, id: 2, name: "other " }],
			},
	...
	it("other name taken", () => {
		const { result } = renderHook(() =>
			useIsSymptomNameAvailable()("other", 2),
		);
		expect(result.current).toBeFalsy();
	});
	...
	it("other name free in cat 1", () => {
		const { result } = renderHook(() =>
			useIsSymptomNameAvailable()("other", 1),
		);
	...
	it("whitespaces are ignored", () => {
		const { result } = renderHook(() =>
			useIsSymptomNameAvailable()(" name ", 1),
		);
		expect(result.current).toBeFalsy();
	});
	...
	it("new name is okay", () => {
		const { result } = renderHook(() =>
			useIsSymptomNameAvailable()("new name", 1),
		);
		expect(result.current).toBeTruthy();
	});
	...
	it("category does not exist", () => {
		const { result } = renderHook(() =>
			useIsSymptomNameAvailable()("new name", 100),
		);
		expect(result.current).toBeFalsy();
	});
});
...
describe("useIsCatNameAvailable", () => {
	...
	it("name taken with whitespace", () => {
		const { result } = renderHook(() =>
			useIsCategoryNameAvailable()(" cat 1 "),
		);
		expect(result.current).toBeFalsy();
	});
	...
	it("name free", () => {
		const { result } = renderHook(() =>
			useIsCategoryNameAvailable()("new cat"),
		);
		expect(result.current).toBeTruthy();
	});
});
