...
export function FoodItem({ food }: { food: FoodItemType }) {
	...
	const onConditionClick = () => {
		actions.meals.patchFoodCondition(
			food.id,
			food.condition === "raw" ? "cooked" : "raw",
		);
	};

	return (
		<SwipeableListItem threshold={0.5} trailingActions={swipeDeleteFood(food)}>
			<ListItem sx={{ pl: "8px", pr: "8px" }}>
				<ListItemText>
					<Typography noWrap>{food.ingredientName}</Typography>
				</ListItemText>
				<NumberField
					sx={{ width: "50px" }}
					size="small"
