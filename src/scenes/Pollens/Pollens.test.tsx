...
describe("Everything is rendered", () => {
	...
	it("Grid is rendered on large screen", async () => {
		...
		await waitFor(() => {
			...
			expect(within(firstRow).getByTestId("pollen-cell-erle")).toHaveStyle({
				"background-color": "greenyellow",
			});
			expect(within(firstRow).getByTestId("pollen-cell-birke")).toHaveStyle({
				"background-color": "orange",
			});
			expect(within(secondRow).getByTestId("pollen-cell-erle")).toHaveStyle({
	...
	it("Grid is rendered on small screen", async () => {
		...
		const firstRow = (await screen.findByText("2.1.2024")).closest(
			"li",
		) as HTMLElement;
		expect(firstRow).toBeInTheDocument();
