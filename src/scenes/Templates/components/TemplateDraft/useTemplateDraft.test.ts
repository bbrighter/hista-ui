...
describe("useTemplate", () => {
	...
	it("ok", () => {
		...
		expect(template?.items).toContainEqual({
			ingredient: { id: 1, name: "Name 1", isArchived: false },
			condition: "raw",
		});
		expect(template?.items).toContainEqual({
			ingredient: { id: 2, name: "Name 2", isArchived: false },
			condition: "cooked",
		});
	});
...
describe("useTemplateDraft", () => {
	...
	it("filled", () => {
		...
		expect(template.draft).toContainEqual({
			ingredient: { id: 1, name: "Name 1", isArchived: false },
			condition: "raw",
		});
		expect(template.draft).toContainEqual({
	...
	it("empty, then edit", () => {
		...
		expect(result.current.draft).toContainEqual({
			ingredient: null,
			condition: "cooked",
		});
		...
		act(() => {
			result.current.onChangeIngredient(0, {
				id: 1,
				name: "Name 1",
				isArchived: false,
			});
