...
type Props = Omit<DialogProps, "onClose"> & { onClose: () => void, templateId?: null | number}
      ...
      <Container sx={{ pt: "6rem" }}>
        ...
        <Stack spacing={1}>
          {draftState.draft.map((_, index) => (
            <DraftRow key={String(draftState.draft[index].ingredient?.id) + draftState.draft[index].condition + index}
              {...draftState} 
              index={index}
            />
          ),
