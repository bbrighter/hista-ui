...
const queryIntakeRows = (name: string): Array<HTMLElement> => {
  ...
  return names.map(n => n.closest("li"))
}
...
describe("Medicines", () => {
  it("Render", async () => {
    render(<MemoryRouter><Medicines/></MemoryRouter>)

    const manageButton = await screen.findByRole("button", { name: "Medikamente verwalten" })
    ...
    rows.forEach((r,i) => {
      ...
      if (i == 0) {
        ...
        expect(within(r).queryByTestId("increaseButton")).not.toBeInTheDocument()
      }
    })
  })
    ...
    render(
      <MemoryRouter initialEntries={["/123/medicines"]}>
        <Routes>
          ...
          <Route    path="/:piid/manage-medicines" element={<>Management scene</>} />
        </Routes>
      </MemoryRouter>,
    )
  ...
  it("Archived medicine not visible", async () => {
    ...
    expect(await waitFor(() => screen.queryByText("Archived medicne"))).not.toBeInTheDocument()
  })
})