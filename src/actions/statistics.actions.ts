...
import {
	respToNutritionStatistics,
	respToRawDiary,
	respToSymptomStatistics,
} from "../store";
...
export const statistics = {
	getMealStatistics: async (
		...
		const resp = await client.GetStatisticsByIngredientId({
	...
	getNutritionStatistics: async (
		...
		setNutritionStatistics({
			interval: interval,
			statistics: respToNutritionStatistics(resp),
		});
	},
};
