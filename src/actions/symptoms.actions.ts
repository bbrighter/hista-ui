...
export const symptoms = {
  ...
  postCategory: async (name: string) => {
    ...
    setSymptoms([...symptoms, { categoryId: resp.id, categoryName: trimmedName, symptoms: [] }])
  ...
  patchSymptomCategory: async (symptomId: number, fromCat: number, toCat: number) => {
    const { setSymptoms, symptoms } = useHista.getState()
    ...
    const relevantSymptom = symptoms[fromCatIdx].symptoms.find(s => s.id === symptomId)
    
    if (fromCatIdx < 0 || toCatIdx < 0 || !relevantSymptom) {
      return
    }
    setSymptoms(symptoms.map(c => {
      if (c.categoryId === fromCat) {
        return { ...c, symptoms: c.symptoms.filter(s => s.id !== symptomId) }
      }
      if (c.categoryId === toCat) {
        return { ...c, symptoms: [...c.symptoms, { ...relevantSymptom, categoryId: toCat }] }
      }
      return c
    }))
