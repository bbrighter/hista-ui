    ...
    server.use(
      http.get("*/permissions", () => {
        return HttpResponse.json({
          ...
          "userName": "user","userId": "1234"        },
        ) 
      }),
    )
    server.use( http.get("http://localhost:4444/piid/:piid/notes", async () => {
      return HttpResponse.json(
        { code: ErrCode.Internal ,message: "message", name: "name",status: 500 } satisfies APIError, 
        { status: 500 },
      )}))
