...
const foodHandlers = (baseUrl: string) => ([
  http.delete(baseUrl + "/foods/:id", () => (HttpResponse.json({ ingredients: [ingredients[1]] } as hista.IngredientListResponse))),
  ...
  http.post(baseUrl + "/meal/:id/foods", async ({ request }) => {
    ...
    return HttpResponse.json(
      {
        food:
                    { id: 1, ingredientId: body.ingredientId, foodCondition: "raw" },
        ingredients:
