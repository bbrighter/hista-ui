...
import type { Symptom, SymptomCategories, SymptomCategory } from "../types";
...
> = (set) => ({
	...
	setSymptoms: (cs: SymptomCategories) => {
		set((state) => {
			state.symptoms = cs;
		});
	},

	addCategory: (c: SymptomCategory) =>
		set((state) => {
	...
	updateCategory: (id: number, part: Partial<SymptomCategory>) =>
		set((state) => {
	...
	updateSymptom: (id: number, part: Partial<Symptom>) =>
		set((state) => {
			const catIdx = state.symptoms.findIndex((c) =>
				c.symptoms.some((s) => s.id === id),
			);
			if (catIdx === -1) return;
			const symptomIdx = state.symptoms[catIdx].symptoms.findIndex(
				(s) => s.id === id,
			);
