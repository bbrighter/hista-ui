...
test("use old intakes", () => {
  ...
  expect(yesterdayIntake.values).toContainEqual({ count: 10, medicineId: 1, name: "Name", visible: true, archived: false })
  ...
  expect(olderIntakes.values).toContainEqual({ count: 0, medicineId: 1, name: "Name", visible: true, archived: false })
  expect(olderIntakes.values).toContainEqual({ count:5, medicineId: 2, name: "Archived", visible: true, archived: true })
})