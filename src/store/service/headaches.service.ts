...
const respToHeadache = (resp: hista.HeadacheResponse): Headache => {
  return  { 
    ...
    id: resp.id,
    positions: resp.positions?.map(p => ({ value: p, label: validHeadachePositions[p]  })) ?? [],
    severity: resp.severity,
    symptoms: resp.symptoms?.map(s => ({ value: s, label: validHeadacheSymptoms[s] })) ?? [],
    types: resp.types?.map(t => ({ value: t, label: validHeadacheTypes[t] })) ?? [],
  }
}

const respToHeadaches = (resp: hista.HeadacheListResponse): Record<number, Headache> => {
...
export const headaches = {
  ...
  post: async (): Promise<number> => {
    ...
    const headache: Headache = { id: resp.id, date: date, severity: severity, positions: [], symptoms: [], types: [], description: "" }
    setHeadache(headache)
...
const equalPositions = (a: Array<{value: string}>, b: Array<{value: string}>) =>
