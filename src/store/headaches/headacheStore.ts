...
import { Headache } from "../types/headaches.types"
...
export const createHeadacheSlice: StateCreator<
  ...
  addHeadache: (id: number, h: Headache) => set(state => {
    state.headaches[id] = h
  }),
  removeHeadache: (id: number) => set(state => {
    state.headaches = Object.fromEntries(
      Object.entries(state.headaches).filter(([i]) => Number(i) != id),
    )
  }),

  setHeadache: (headache: Headache) => set(state => {
    state.headache = headache
  }),
  setHeadaches: (headaches: Record<number, Headache>) => set(state => {
    state.headaches = headaches
  }),
  updateHeadache: (id: number, part: Partial<Headache>) => set(state => {
